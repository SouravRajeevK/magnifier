(function (window) {
    ...
    function HTMLMagnifier(options) {
        ...
        const magnifierTemplate = `
            <div class="accessibility-toolbar-magnifier" style="display: none; position: fixed; box-shadow: rgba(0, 0, 0, 0.25) 0px 54px 55px, rgba(0, 0, 0, 0.12) 0px -12px 30px, rgba(0, 0, 0, 0.12) 0px 4px 6px, rgba(0, 0, 0, 0.17) 0px 12px 13px, rgba(0, 0, 0, 0.09) 0px -3px 5px; overflow: hidden; background-color: white; border: 1px solid #555; border-radius: 4px; z-index: 10000;">
            <div class="accessibility-toolbar-magnifier-content" style="top: 0px; left: 0px; margin-left: 0px; margin-top: 0px; overflow: visible; position: absolute; display: block; transform-origin: left top; user-select: none; padding-top: 0px;"></div>
            <div class="accessibility-toolbar-magnifier-glass" style="position: absolute; top: 0px; left: 0px; width: 100%; height: 100%; opacity: 0.0; background-color: white; cursor: move;"></div>
            <div class="accessibility-toolbar-magnifier-controls" style="position: absolute; bottom: 5px; left: 5px; right: 5px; height: 30px; background-color: rgba(0, 0, 0, 0.5); border-radius: 3px; display: flex; align-items: center; justify-content: space-between; padding: 0 10px; z-index: 10001;">
                <button class="accessibility-toolbar-zoom-out" style="background: none; border: none; color: white; font-size: 16px; cursor: pointer; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center;">−</button>
                <div class="accessibility-toolbar-zoom-level" style="color: white; font-size: 12px;"></div>
                <button class="accessibility-toolbar-zoom-in" style="background: none; border: none; color: white; font-size: 16px; cursor: pointer; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center;">+</button>
            </div>

            <div style="position: absolute; top: 50%; left: 50%; width: 4px; height: 4px; background-color: red; border-radius: 50%; transform: translate(-50%, -50%); z-index: 10002;"></div>
            </div>
        `;
        ...
        function setupMagnifier() { // Rebuild the magnifier
            if (!isManuallyMoved) { // Only set position if user hasn't moved it
        ...
        function updateMagnifiedArea() {
            ...
            const centerX = _this.options.width / 2;//pos to center mouse
            const centerY = _this.options.height / 2;

            const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft || 0;//scroll offsets
            const scrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;

            const x = scrollLeft + mousePosition.x;
            const y = scrollTop + mousePosition.y;

            const contentLeft = -x * _this.options.zoom + centerX;//pos content
        ...
        function removeSelectors(container, selector) {//removing a cntent 
            const elements = container.querySelectorAll(selector);
            elements.forEach(el => el.parentNode.removeChild(el));
        }//here to remove the magnifier from cloned dom

        function prepareContent() {
            magnifierContent.innerHTML = '';

            const bodyOriginal = document.documentElement;//cloning the dom
            ...
            const canvasOriginal = bodyOriginal.querySelectorAll('canvas');// Copy canvas content if present
            ...
            if (canvasOriginal.length > 0 && canvasOriginal.length === canvasCopy.length) {
                for (let i = 0; i < canvasOriginal.length; i++) {
                    try {
                        ...
                        console.log('Failed to copy canvas content');
                    }
                }
            }

            ['.accessibility-toolbar-magnifier'].forEach(selector => {
                removeSelectors(bodyCopy, selector);// Remove magnifer from cloned dom
            });
        ...
        document.addEventListener("DOMContentLoaded", () => {//listennig dom update events
        ...
        function init() {
            ...
            magnifier = div.querySelector('.accessibility-toolbar-magnifier');
            document.body.appendChild(magnifier);

            // Get references to elements
            magnifierContent = magnifier.querySelector('.accessibility-toolbar-magnifier-content');
            magnifierGlass = magnifier.querySelector('.accessibility-toolbar-magnifier-glass');
            magnifierControls = magnifier.querySelector('.accessibility-toolbar-magnifier-controls');
            zoomInBtn = magnifier.querySelector('.accessibility-toolbar-zoom-in');
            zoomOutBtn = magnifier.querySelector('.accessibility-toolbar-zoom-out');
            zoomLevelDisplay = magnifier.querySelector('.accessibility-toolbar-zoom-level');
